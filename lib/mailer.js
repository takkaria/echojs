...
var emailTemplates = Promise.denodeify(require('swig-email-templates-2'));
var path = require('path');
var debug = require('debug')('echo:mailer');
var logger = require('./logger');

var mailTemplateOptions = {
...
function sendMail(mail) {
	...
	return emailTemplates(mailTemplateOptions).then(function(render) {
		return Promise.denodeify(render)(mail.template, mail.context, null);
	}).then(function(html) {
...
module.exports = {
	...
	sendAdminsEventNotifyMail: function (models, event_) {
		return models.User.findAll({
			...
			var emails_ = emails.map(function(value, i, array) {
				return value.email;
			});
			debug(emails_.join(','));
			return sendMail({
