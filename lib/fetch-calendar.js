...
function requestNoteRedirects(thisUrl, opts, cb) {
	let newLocation = thisUrl;
	...
	let redirectOpts = {
		followRedirect: response => {
			if (!isTemporary && response.statusCode === 301) {
				newLocation = url.resolve(thisUrl, response.headers.location);
				hasRedirected = true;
			} else {
				isTemporary = true;
			}
	...
	request(thisUrl, newOpts, (err, res, data) => {
		res.redirects = {
...
function getCalendar(url, cb) {
	requestNoteRedirects(url, null, function(err, res, data) {
		if (err) {
			return cb(err, null);
		}
