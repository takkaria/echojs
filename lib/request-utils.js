...
function requestNoteRedirects(opts, cb) {
	...
	let redirectOpts = {
		followRedirect: response => {
			if (!isTemporary && response.statusCode === 301) {
				newLocation = url.resolve(newLocation, response.headers.location);
				hasRedirected = true;
			} else {
				isTemporary = true;
			}
	...
	request(newOpts, (err, res, data) => {
		if (res && hasRedirected) {
			res.newLocation = newLocation;
		}

		res.cacheData = extractCacheData(res.headers);
